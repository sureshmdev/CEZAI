...
export default function Quiz() {
  ...
  } = useFetch<RawAssessmentAssessment, [QuizQuestionInput[], string[], number]>(
    saveQuizResult
  );
  ...
  const finishQuiz = async () => {
    ...
    try {
      ...
      // Map JsonValue[] safely to QuizResultInput[]
      const mappedQuestions: QuizResultInput[] = (
        result.questions as unknown as Array<Record<string, unknown>>
      ).map((q) => {
        if (
          q &&
          "question" in q &&
      ...
      // Store mapped result as Assessment
      setResultData({
        ...result,

...
// --- Type guard for QuizResultInput ---
function isQuizResultInput(q: JsonValue): q is QuizResultInput {
  return (
    typeof q === "object" &&
    q !== null &&
    "question" in q &&
...
// --- Mapper: RawAssessment -> Assessment ---
function mapToAssessment(raw: RawAssessment): Assessment {
  const mappedQuestions: QuizResultInput[] = raw.questions.map((q) => {
...
export default function Quiz() {
  ...
  const {
    ...
    fn: saveQuizResultFn,
    data: resultData, // RawAssessment | undefined
  ...
  const finishQuiz = async () => {
    ...
    try {
      const rawResult = await saveQuizResultFn(quizData, answers, score);
      setResultData(rawResult); // ✅ keep RawAssessment in state
  ...
  if (resultData) {
    const assessment: Assessment = mapToAssessment(resultData);
